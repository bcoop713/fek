import {isString, isNumber, Maybe, validate, mapValidator, Obj, List, Tuple} from '../src/schema'
...
describe('tuple', () => {
  ...
  test('error', () => {
    ...
    const result = validate(validator)(data)
    const expected = Error([{expected: 'number', received: 'string1', path: [1]}])
    expect(result).toEqual(expected);
  })
  test('missing values', () => {
    const data = [1];
    const validator = Tuple([isNumber, isNumber]);
    const result = validate(validator)(data)
    const expected = Error([{expected: 'number', received: undefined, path: [1]}])
    expect(result).toEqual(expected);
  })
})
...
describe('coercion', () => {
  ...
  test('error', () => {
    ...
    const validator = Maybe(isNumber)
    const result = validate(validator)(data)
    const expected = Error({expected: 'number', received: 'string1', path: []});
    expect(result).toEqual(expected)
  })
